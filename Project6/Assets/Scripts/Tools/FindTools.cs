...
namespace Tools
{
    public class FindTools : MonoBehaviour
    {
        ...
        public static List<Transform> FindChilds(Transform parent, string targetName)
        {
            ...
            List<GameObject> allChilds = FindAllChilds(parent.gameObject);

            foreach (var child in allChilds)
            {
                if (child.name.Equals(targetName)) result.Add(child.transform);
            }
        ...
        public static List<GameObject> FindAllChilds(GameObject parent)
        {
            ...
            //利用for循环 获取物体下的全部子物体
            for (int t = 0; t < parent.transform.childCount; t++)
            {
                //如果子物体下还有子物体 就将子物体传入进行回调查找 直到物体没有子物体为止
                if (parent.transform.GetChild(t).childCount > 0)
                {
                    FindAllChilds(parent.transform.GetChild(t).gameObject);
                }
