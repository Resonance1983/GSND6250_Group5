...
uniform float4 _CloudsCoverageSettings;
//x = _GlobalCoverage, y = Bottom Coverage Mod, z = Top coverage mod, w = Clouds Up Morph Intensity
...
float HeightAlter(float percent_height, float weather, float anvil)
{
    ...
    ret_val = pow(ret_val, saturate(RemapEnviro(percent_height, 0.65, 0.95, 1.0,
...
float CalculateCloudDensity(float3 pos, float3 PlanetCenter, RaymarchParameters parameters, float2 weather, float mip,
                            float lod, bool details)
{
...
float GetDensityAlongRay(float3 pos, float3 PlanetCenter, RaymarchParameters parameters, float3 LightDirection,
                         float2 weather, float lod)
{
...
float SampleEnergy(float3 pos, float cosTheta, float3 cent, RaymarchParameters parameters, float3 LightDirection,
                   float height, float ds_loded, float step_size, float2 weather, float lod)
{
    ...
    for (int octaveIndex = 0; octaveIndex < 2; octaveIndex++)
    {
        ...
        float phase = lerp(HenryGreenstein(cosTheta, .5 * ecMult),
                           HenryGreenstein(cosTheta, (0.99 - parameters.silverLiningSpread) * ecMult), 0.5f);
        result += phase * transmittance * parameters.scatteringCoef * 25.0f * powder * pow(
            parameters.multiScatteringA, octaveIndex);
    }
...
bool resolve_ray_start_end(float3 ws_origin, float3 ws_ray, float3 center, RaymarchParameters parameter,
                           out float start, out float end)
{
    //case includes on ground, inside atm, above atm.
    float ot1, ot2, it1, it2;
    bool outIntersected = ray_trace_sphere(ws_origin, ws_ray, center,
                                           parameter.cloudsParameter.w + parameter.cloudsParameter.y, ot1, ot2);
    if (!outIntersected || ot2 < 0.0f)
        return false; //you see nothing.

    bool inIntersected = ray_trace_sphere(ws_origin, ws_ray, center,
                                          parameter.cloudsParameter.w + parameter.cloudsParameter.x, it1, it2);
...
float3 Raymarch(float3 cameraPos, float3 ray, float2 hitDistance, float3 center, RaymarchParameters parameters,
                float offset, int layer)
{
    ...
    for (int i = 0; i < steps; i++)
    {
        ...
        if (cloud_test > 0.0)
        {
            ...
            if (zero_density_sample_count < 8 && sampled_density != 0.0)
            {
                ...
                float luminance = SampleEnergy(pos, cosTheta, center, parameters, _LightDir, height, sampled_density,
                                               rayStepLength, weather, lod);
...
float RaymarchShadows(float3 cameraPos, float3 worldPos, float3 ray, float3 center, RaymarchParameters parameters,
                      float offset, float depth, int layer)
{
    if (depth == 0.0f)
        return 0.0;
