...
float4 SampleAurora(float3 uv)
{
    ...
    float3 uv_3 = float3(uv.xy * _Aurora_Tiling_ColorShift.xy + (_Aurora_Tiling_ColorShift.zw * _AuroraSpeed * _Time.y),
                         0.0);
...
float4 Aurora(float3 wpos)
{
    ...
    float3 worldPos = _WorldSpaceCameraPos + traceDir * ((_AuroraHeight - _WorldSpaceCameraPos.y) / max(
        traceDir.y, 0.01));
