...
namespace Enviro
{
    ...
    public class EnviroEnvironment
    {
        ...
        [Tooltip("Start Day of Year for Spring")] [Range(0, 366)]
        public int springStart = 60;

        [Tooltip("End Day of Year for Spring")] [Range(0, 366)]
        public int springEnd = 92;

        [Tooltip("Start Day of Year for Summer")] [Range(0, 366)]
        public int summerStart = 93;

        [Tooltip("End Day of Year for Summer")] [Range(0, 366)]
        public int summerEnd = 185;

        [Tooltip("Start Day of Year for Autumn")] [Range(0, 366)]
        public int autumnStart = 186;

        [Tooltip("End Day of Year for Autumn")] [Range(0, 366)]
        public int autumnEnd = 276;

        [Tooltip("Start Day of Year for Winter")] [Range(0, 366)]
        public int winterStart = 277;

        [Tooltip("End Day of Year for Winter")] [Range(0, 366)]
        public int winterEnd = 59;

        //Temperature
        [Tooltip("Base Temperature in Spring")]
        public AnimationCurve springBaseTemperature = new();

        [Tooltip("Base Temperature in Summer")]
        public AnimationCurve summerBaseTemperature = new();

        [Tooltip("Base Temperature in Autumn")]
        public AnimationCurve autumnBaseTemperature = new();

        [Tooltip("Base Temperature in Winter")]
        public AnimationCurve winterBaseTemperature = new();

        [Tooltip("Current temperature.")] [Range(-50f, 50f)]
        public float temperature = 0f;

        [Tooltip("Temperature mod used for different weather types.")] [Range(-50f, 50f)]
        public float temperatureWeatherMod = 0f;

        [Tooltip("Custom temperature mod for gameplay use.")] [Range(-50f, 50f)]
        ...
        //Weather State
        [Tooltip("Current wetness for third party shader or gameplay.")] [Range(0f, 1f)]
        public float wetness = 0f;

        [Tooltip("Target wetness for third party shader or gameplay.")] [Range(0f, 1f)]
        public float wetnessTarget = 0f;

        [Tooltip("Current snow for third party shader or gameplay.")] [Range(0f, 1f)]
        public float snow = 0f;

        [Tooltip("Target snow for third party shader or gameplay.")] [Range(0f, 1f)]
        public float snowTarget = 0f;

        [Tooltip("Speed of wetness accumulation.")]
        public float wetnessAccumulationSpeed = 1f;

        [Tooltip("Speed of wetness dries.")] public float wetnessDrySpeed = 1f;

        [Tooltip("Speed of snow buildup.")] public float snowAccumulationSpeed = 1f;

        [Tooltip("Speed of how fast snow melts.")]
        public float snowMeltSpeed = 1f;

        [Tooltip("Temperature when snow starts to melt.")] [Range(-20f, 20f)]
        public float snowMeltingTresholdTemperature = 1f;

        //Wind 
        [Range(-1f, 1f)] public float windDirectionX, windDirectionY;
        [Range(0f, 1f)] public float windSpeed = 0.1f;
        [Range(0f, 1f)] public float windTurbulence = 0.1f;
    }
    ...
    public class EnviroEnvironmentModule : EnviroModule
    {
        ...
        public void UpdateSeason()
        {
            ...
            {
                var currentDay = EnviroManager.instance.Time.Settings.date.DayOfYear;
        ...
        public void UpdateTemperature(float timeOfDay)
        {
            ...
            //Set temperature
            Settings.temperature = Mathf.Lerp(Settings.temperature, temperature,
                Time.deltaTime * Settings.temperatureChangingSpeed);
        }
        ...
        public void UpdateWeatherState()
        {
            ...
            if (Settings.wetness < Settings.wetnessTarget)
                // Raining
                Settings.wetness = Mathf.Lerp(Settings.wetness, Settings.wetnessTarget,
                    Settings.wetnessAccumulationSpeed * Time.deltaTime);
            else
                // Drying
                Settings.wetness = Mathf.Lerp(Settings.wetness, Settings.wetnessTarget,
                    Settings.wetnessDrySpeed * Time.deltaTime);
            ...
            if (Settings.snow < Settings.snowTarget)
                //Snowing
                Settings.snow = Mathf.Lerp(Settings.snow, Settings.snowTarget,
                    Settings.snowAccumulationSpeed * Time.deltaTime);
        ...
        private void UpdateWindZone()
        {
            if (EnviroManager.instance.Objects.windZone != null)
            {
                ...
                var windDirection = new Vector3(-Settings.windDirectionX, 0f, -Settings.windDirectionY);
        ...
        {
#if UNITY_EDITOR
            var t = CreateInstance<EnviroEnvironmentModule>();
            t.name = "Environment Preset";
            t.Settings = JsonUtility.FromJson<EnviroEnvironment>(JsonUtility.ToJson(Settings));
