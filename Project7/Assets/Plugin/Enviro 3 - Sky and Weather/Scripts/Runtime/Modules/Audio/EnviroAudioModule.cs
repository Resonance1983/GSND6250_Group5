...
namespace Enviro
{
    ...
    {
        public List<EnviroAudioClip> ambientClips = new();
        public List<EnviroAudioClip> weatherClips = new();
        public List<EnviroAudioClip> thunderClips = new();
    ...
    public class EnviroAudioModule : EnviroModule
    {
        ...
        public void CreateAudio()
        {
            ...
            //Ambient
            for (var i = 0; i < Settings.ambientClips.Count; i++)
            {
                ...
                if (Settings.ambientClips[i].audioClip != null)
                {
                    ...
                    Settings.ambientClips[i].myAudioSource.outputAudioMixerGroup =
                        Settings.ambientClips[i].audioMixerGroup;
                }
            }

            //Weather
            for (var i = 0; i < Settings.weatherClips.Count; i++)
            {
                ...
                if (Settings.weatherClips[i].audioClip != null)
                {
                    ...
                    Settings.weatherClips[i].myAudioSource.outputAudioMixerGroup =
                        Settings.weatherClips[i].audioMixerGroup;
                }
            }

            //Tunder
            for (var i = 0; i < Settings.thunderClips.Count; i++)
            {
                ...
                if (Settings.thunderClips[i].audioClip != null)
                {
                    ...
                    Settings.thunderClips[i].myAudioSource.outputAudioMixerGroup =
                        Settings.thunderClips[i].audioMixerGroup;
                }
            }
        }
        ...
        {
            var thunderSFX = UnityEngine.Random.Range(0, Settings.thunderClips.Count);

            if (Settings.thunderClips.Count > 0 && Settings.thunderClips[thunderSFX] != null)
            {
                Settings.thunderClips[thunderSFX].myAudioSource.volume =
                    Settings.thunderClips[thunderSFX].volume * Settings.thunderMasterVolume + thunderVolumeModifier;
                Settings.thunderClips[thunderSFX].myAudioSource
                    .PlayOneShot(Settings.thunderClips[thunderSFX].myAudioSource.clip);
            }
        }
        ...
        {
#if UNITY_EDITOR
            var t = CreateInstance<EnviroAudioModule>();
            t.name = "Audio Module";
            t.Settings = JsonUtility.FromJson<EnviroAudio>(JsonUtility.ToJson(Settings));
