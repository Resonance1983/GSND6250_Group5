...
namespace Enviro
{
    ...
    {
        public List<EnviroWeatherType> weatherTypes = new();
    ...
    public class EnviroWeatherModule : EnviroModule
    {
        ...
        {
            var type = EnviroWeatherTypeCreation.CreateMyAsset();
        ...
        {
            for (var i = 0; i < Settings.weatherTypes.Count; i++)
        ...
        {
            var lighting = EnviroManager.instance.Lighting;
            ...
            if (lighting != null)
            {
                ...
                lighting.Settings.shadowIntensity = Mathf.Lerp(lighting.Settings.shadowIntensity,
                    targetWeatherType.lightingOverride.shadowIntensity, blendTime);
            }
        }

        private void BlendSkyOverride(float blendTime)
        {
            var sky = EnviroManager.instance.Sky;

            if (sky != null)
                //sky.Settings.intensity = Mathf.Lerp(sky.Settings.intensity, targetWeatherType.skyOverride.intensity,blendTime); 
                sky.Settings.skyColorTint = Color.Lerp(sky.Settings.skyColorTint,
                    targetWeatherType.skyOverride.skyColorTint, blendTime);
        }

        private void BlendFogOverride(float blendTime)
        {
            var fog = EnviroManager.instance.Fog;
            ...
            if (fog != null)
            {
                ...
                fog.Settings.fogColorMod = Color.Lerp(fog.Settings.fogColorMod,
                    targetWeatherType.fogOverride.fogColorMod, blendTime);
                ...
                fog.Settings.unityFogDensity = Mathf.Lerp(fog.Settings.unityFogDensity,
                    targetWeatherType.fogOverride.unityFogDensity, blendTime);
                fog.Settings.unityFogStartDistance = Mathf.Lerp(fog.Settings.unityFogStartDistance,
                    targetWeatherType.fogOverride.unityFogStartDistance, blendTime);
                fog.Settings.unityFogEndDistance = Mathf.Lerp(fog.Settings.unityFogEndDistance,
                    targetWeatherType.fogOverride.unityFogEndDistance, blendTime);
            }
        }

        private void BlendEffectsOverride(float blendTime)
        {
            var effects = EnviroManager.instance.Effects;

            if (effects != null)
                for (var i = 0; i < effects.Settings.effectTypes.Count; i++)
                {
                    var hasOverride = false;

                    for (var a = 0; a < targetWeatherType.effectsOverride.effectsOverride.Count; a++)
                        if (effects.Settings.effectTypes[i].name ==
                            targetWeatherType.effectsOverride.effectsOverride[a].name)
                        {
                            effects.Settings.effectTypes[i].emissionRate = Mathf.Lerp(
                                effects.Settings.effectTypes[i].emissionRate,
                                targetWeatherType.effectsOverride.effectsOverride[a].emission, blendTime);
                            hasOverride = true;
                        }

                    if (!hasOverride)
                        effects.Settings.effectTypes[i].emissionRate =
                            Mathf.Lerp(effects.Settings.effectTypes[i].emissionRate, 0f, blendTime);
                }
        }

        private void BlendVolumetricCloudsOverride(float blendTime)
        {
            var clouds = EnviroManager.instance.VolumetricClouds;

            if (clouds != null)
            {
                clouds.settingsGlobal.ambientLighIntensity = Mathf.Lerp(clouds.settingsGlobal.ambientLighIntensity,
                    targetWeatherType.cloudsOverride.ambientLightIntensity, blendTime);
                ...
                clouds.settingsLayer1.densitySmoothness = Mathf.Lerp(clouds.settingsLayer1.densitySmoothness,
                    targetWeatherType.cloudsOverride.densitySmoothnessLayer1, blendTime);
                ...
                if (clouds.settingsGlobal.dualLayer)
                {
                    ...
                    clouds.settingsLayer2.densitySmoothness = Mathf.Lerp(clouds.settingsLayer2.densitySmoothness,
                        targetWeatherType.cloudsOverride.densitySmoothnessLayer2, blendTime);
        ...
        {
            var flatClouds = EnviroManager.instance.FlatClouds;
        ...
        {
            var aurora = EnviroManager.instance.Aurora;
        ...
        {
            var environment = EnviroManager.instance.Environment;
        ...
        {
            var audio = EnviroManager.instance.Audio;

            if (audio != null)
            {
                for (var i = 0; i < audio.Settings.ambientClips.Count; i++)
                {
                    var hasOverride = false;

                    for (var a = 0; a < targetWeatherType.audioOverride.ambientOverride.Count; a++)
                ...
                for (var i = 0; i < audio.Settings.weatherClips.Count; i++)
                {
                    var hasOverride = false;

                    for (var a = 0; a < targetWeatherType.audioOverride.weatherOverride.Count; a++)
        ...
        {
            var lightning = EnviroManager.instance.Lightning;
        ...
        public void ChangeZoneWeather(int weather, int zone)
        {
            if (EnviroManager.instance.zones.Count >= zone && Settings.weatherTypes.Count >= weather)
            {
                ...
                EnviroManager.instance.NotifyZoneWeatherChanged(Settings.weatherTypes[weather],
                    EnviroManager.instance.zones[zone]);
            }
        }
        ...
        {
#if UNITY_EDITOR
            var t = CreateInstance<EnviroWeatherModule>();
            t.name = "Weather Module";
            t.Settings = JsonUtility.FromJson<EnviroWeather>(JsonUtility.ToJson(Settings));
