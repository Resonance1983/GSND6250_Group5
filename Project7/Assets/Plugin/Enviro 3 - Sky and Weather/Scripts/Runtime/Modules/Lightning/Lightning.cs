...
*/
        ...
        private IEnumerator CreateSplit(Vector3 pos, Vector3 targetP)
        {
            ...
            var splitRenderer = split.AddComponent<LineRenderer>();
            ...
            var posDown = new Vector3(toTarget.x, toTarget.y, toTarget.z * 0.1f);
            var targetPos = Random.insideUnitSphere * 500 + pos + toTarget * 500;

            var lastPoint = split.transform.position;
            var dist = Vector3.Distance(split.transform.position, targetPos);
            ...
            for (var i = 1; i < 8; i++)
            {
                ...
                var fwd = targetPos - lastPoint;
                fwd.Normalize();
                var newPos = Randomize(fwd, inaccuracy);
        ...
        {
            newVector += new Vector3(Random.Range(-1.0f, 1.0f), Random.Range(-1.0f, 1.0f), Random.Range(-1.0f, 1.0f)) *
                         devation;
            newVector.Normalize();
            return newVector;
        }
