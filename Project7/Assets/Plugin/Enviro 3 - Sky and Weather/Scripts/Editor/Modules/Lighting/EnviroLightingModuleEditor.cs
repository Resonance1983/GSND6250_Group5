...
namespace Enviro
{
    ...
    public class EnviroLightingModuleEditor : EnviroModuleEditor
    {
        ...
        //Properties 
        //Direct Lighting
        private SerializedProperty updateIntervallFrames,
            directLightIntensityModifier,
            sunIntensityCurve,
            moonIntensityCurve,
            sunColorGradient,
            moonColorGradient,
        ...
        private SerializedProperty shadowIntensity,
            ambientIntensityModifier,
            ambientMode,
            ambientSkyboxUpdateIntervall,
...
*/
        public override void OnInspectorGUI()
        {
            ...
            {
                EnviroManager.instance.RemoveModule(EnviroManagerBase.ModuleType.Lighting);
            ...
            if (myTarget.showModuleInspector)
            {
                ...
                GUILayout.BeginVertical("", boxStyleModified);
                GUI.backgroundColor = Color.white;
                myTarget.showDirectLightingControls = GUILayout.Toggle(myTarget.showDirectLightingControls,
                    "Direct Lighting Controls", headerFoldout);
                if (myTarget.showDirectLightingControls)
                {
                    GUILayout.Space(10);
                    myTarget.Settings.setDirectLighting =
                    ...
                    if (GUILayout.Button("Apply Lighting Mode Changes")) myTarget.ApplyLightingChanges();
                    GUILayout.Space(10);
                ...
                GUILayout.BeginVertical("", boxStyleModified);
                GUI.backgroundColor = Color.white;
                myTarget.showAmbientLightingControls = GUILayout.Toggle(myTarget.showAmbientLightingControls,
                    "Ambient Lighting Controls", headerFoldout);
                if (myTarget.showAmbientLightingControls)
                {
                    GUILayout.Space(10);

                    myTarget.Settings.setAmbientLighting = EditorGUILayout.BeginToggleGroup("Set Ambient Lighting",
                        myTarget.Settings.setAmbientLighting);
#if !ENVIRO_HDRP
