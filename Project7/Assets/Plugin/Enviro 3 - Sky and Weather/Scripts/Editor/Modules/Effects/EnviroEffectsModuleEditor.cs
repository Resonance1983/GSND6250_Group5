...
namespace Enviro
{
    ...
    public class EnviroEffectsModuleEditor : EnviroModuleEditor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                EnviroManager.instance.RemoveModule(EnviroManagerBase.ModuleType.Effects);
            ...
            if (myTarget.showModuleInspector)
            {
                ...
                if (myTarget.showSetupControls)
                {
                    ...
                    {
                        if (GUILayout.Button("Add")) myTarget.Settings.effectTypes.Add(new EnviroEffectTypes());
                    }
                    else
                    {
                    ...
                    if (GUILayout.Button("Apply Changes")) myTarget.CreateEffects();

                    GUILayout.Space(10);

                    for (var i = 0; i < myTarget.Settings.effectTypes.Count; i++)
                    {
                        ...
                        myTarget.Settings.effectTypes[i].name =
                            EditorGUILayout.TextField("Effect Name", myTarget.Settings.effectTypes[i].name);
                        myTarget.Settings.effectTypes[i].prefab = (GameObject)EditorGUILayout.ObjectField(
                            "Effect Prefab", myTarget.Settings.effectTypes[i].prefab, typeof(GameObject), true);
                        myTarget.Settings.effectTypes[i].localPositionOffset =
                            EditorGUILayout.Vector3Field("Position Offset",
                                myTarget.Settings.effectTypes[i].localPositionOffset);
                        myTarget.Settings.effectTypes[i].localRotationOffset =
                            EditorGUILayout.Vector3Field("Rotation Offset",
                                myTarget.Settings.effectTypes[i].localRotationOffset);
                        GUILayout.Space(10);
                        myTarget.Settings.effectTypes[i].emissionRate = EditorGUILayout.Slider("Emission",
                            myTarget.Settings.effectTypes[i].emissionRate, 0f, 1f);
                        myTarget.Settings.effectTypes[i].maxEmission = EditorGUILayout.FloatField("Maximum Emission",
                            myTarget.Settings.effectTypes[i].maxEmission);
