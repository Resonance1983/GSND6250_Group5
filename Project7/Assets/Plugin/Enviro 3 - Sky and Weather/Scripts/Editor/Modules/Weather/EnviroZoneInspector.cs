    ...
    public class EnviroZoneInspector : EnviroBaseInspector
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (myTarget.nextWeatherType != null)
            {
                if (EnviroManager.instance != null && EnviroManager.instance.Time != null)
                {
                    if (EnviroManager.instance.Weather != null &&
                        EnviroManager.instance.Weather.globalAutoWeatherChange && myTarget.autoWeatherChanges)
                        GUILayout.Label(
                            "Next Change in: " +
                            (myTarget.nextWeatherUpdate - EnviroManager.instance.Time.GetDateInHours())
                            .ToString("#.00") + " hours", wrapStyle);
                    else
            ...
            if (commandName == "ObjectSelectorClosed")
            {
                ...
                for (var i = 0; i < myTarget.weatherTypeList.Count; i++)
            ...
            for (var i = 0; i < myTarget.weatherTypeList.Count; i++)
            {
                var curZoneWeather = myTarget.weatherTypeList[i];
                GUILayout.BeginVertical("", boxStyleModified);
                ...
                if (curZoneWeather.weatherType != myTarget.currentWeatherType)
                    if (GUILayout.Button("Change Now", EditorStyles.miniButtonRight, GUILayout.Width(80),
                            GUILayout.Height(18)))
                ...
                if (curZoneWeather.showEditor)
                {
                    ...
                    if ((EnviroManager.instance != null && EnviroManager.instance.Environment != null) ||
                        EnviroManager.instance == null)
                        curZoneWeather.seasonalProbability = EditorGUILayout.Toggle("Seasonal Probabillity",
                            curZoneWeather.seasonalProbability);

                    EditorGUILayout.Space(5f);

                    if (curZoneWeather.seasonalProbability == false ||
                        (EnviroManager.instance != null && EnviroManager.instance.Environment == null))
                    {
                        curZoneWeather.probability =
                            EditorGUILayout.Slider("Probabillity", curZoneWeather.probability, 0f, 100f);
                    }
                    else if ((EnviroManager.instance != null && EnviroManager.instance.Environment != null) ||
                             EnviroManager.instance == null)
                    {
                        curZoneWeather.probabilitySpring = EditorGUILayout.Slider("Spring Probabillity",
                            curZoneWeather.probabilitySpring, 0f, 100f);
                        curZoneWeather.probabilitySummer = EditorGUILayout.Slider("Summer Probabillity",
                            curZoneWeather.probabilitySummer, 0f, 100f);
                        curZoneWeather.probabilityAutumn = EditorGUILayout.Slider("Autumn Probabillity",
                            curZoneWeather.probabilityAutumn, 0f, 100f);
                        curZoneWeather.probabilityWinter = EditorGUILayout.Slider("Winter Probabillity",
                            curZoneWeather.probabilityWinter, 0f, 100f);
                    }
